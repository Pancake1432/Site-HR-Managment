...
export default function DocumentsPage() {
  ...
  // Docs for the grid cards (map of driverId → DriverDocSet)
  const [gridDocs, setGridDocs] = useState<Record<number, DriverDocSet>>({});
  // Docs for the open modal
  const [modalDocs, setModalDocs] = useState<DriverDocSet>(EMPTY_DOCS);
  ...
  useEffect(() => {
    if (id) {
      ...
      setModalDocs(EMPTY_DOCS);
    }
  }, [id, allApplicantsData]);

  // Load docs for modal when selectedDriver changes
  useEffect(() => {
    if (!selectedDriver) return;
    getDriverDocs(selectedDriver.id).then(setModalDocs);
  }, [selectedDriver, getDriverDocs]);

  // Load docs for grid cards when applicants load
  useEffect(() => {
    if (allApplicantsData.length === 0) return;
    const loadAll = async () => {
      const entries = await Promise.all(
        allApplicantsData.map(async d => [d.id, await getDriverDocs(d.id)] as [number, DriverDocSet])
      );
      setGridDocs(Object.fromEntries(entries));
    };
    loadAll();
  }, [allApplicantsData, getDriverDocs]);
  ...
  const handleUpload = async (type: 'cdl' | 'medicalCard', file: File | undefined) => {
  ...
  const handleDelete = async (type: 'cdl' | 'medicalCard') => {
    if (!selectedDriver) return;
    const doc = modalDocs[type];
    if (!doc) return;
  ...
  const handleDeleteDriver = useCallback(async () => {
    ...
    if (window.confirm(`Delete ${selectedDriver.name} and all their documents?\n\nThis cannot be undone.`)) {
      await deleteApplicant(selectedDriver.id);
      navigate('/dashboard/documents');
      refresh();
    }
  }, [selectedDriver, navigate, refresh]);

  const handleEquipmentChange = useCallback(async (driverId: number, equipment: EquipmentType) => {
    const current = allApplicantsData.find(a => a.id === driverId);
    await saveApplicantOverride(driverId, { equipment, status: current?.status });
    if (selectedDriver?.id === driverId) setSelectedDriver(prev => prev ? { ...prev, equipment } : prev);
    refresh();
  }, [refresh, selectedDriver, allApplicantsData]);

  const handleStatusChange = useCallback(async (driverId: number, status: StatusType) => {
    await saveApplicantOverride(driverId, { status });
  ...
  const handleHire = useCallback(async () => {
    ...
    if (window.confirm(`Hire ${selectedDriver.name}?\n\nThey will be moved to the Drivers section.`)) {
      await hireApplicant(selectedDriver);
      navigate('/dashboard/documents');
      refresh();
    }
  }, [selectedDriver, navigate, refresh]);
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        <div className="documents-grid">
          {filtered.length > 0 ? filtered.map(d => {
            const docs  = gridDocs[d.id] ?? EMPTY_DOCS;
              ...
              <div key={d.id} className="document-card">
                <div className="document-card-content">
                  ...
                  <div className="document-card-stats" style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '12px', padding: '2px 8px', borderRadius: '12px', background: docs.applicationPdf ? '#c6f6d5' : '#edf2f7', color: docs.applicationPdf ? '#276749' : '#718096' }}>Form {docs.applicationPdf ? '✓' : '–'}</span>
                    <span style={{ fontSize: '12px', padding: '2px 8px', borderRadius: '12px', background: docs.cdl ? '#c6f6d5' : '#fed7d7', color: docs.cdl ? '#276749' : '#9b2c2c' }}>CDL {docs.cdl ? '✓' : '✗'}</span>
                    <span style={{ fontSize: '12px', padding: '2px 8px', borderRadius: '12px', background: docs.medicalCard ? '#c6f6d5' : '#fed7d7', color: docs.medicalCard ? '#276749' : '#9b2c2c' }}>Medical {docs.medicalCard ? '✓' : '✗'}</span>
                  </div>
      ...
      <input ref={cdlInputRef} type="file" accept=".pdf,application/pdf,image/*" onChange={e => handleUpload('cdl', e.target.files?.[0])} style={{ display: 'none' }} />
      <input ref={medInputRef} type="file" accept=".pdf,application/pdf,image/*" onChange={e => handleUpload('medicalCard', e.target.files?.[0])} style={{ display: 'none' }} />
        ...
        <div className="modal-overlay" onClick={handleClose}>
          <div className="modal-content modal-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="modal-section">
                <div className="modal-section-header"><h3>Documents ({filledDocCount(modalDocs)}/3)</h3></div>
                <div className="driver-documents-grid">
                  {slotDefs.map(({ key, icon, label, readOnly, ref }) => {
                    const doc = modalDocs[key];
                        ...
                        {doc ? (
                          <div className="driver-doc-info">
                            ...
                            <div className="driver-doc-actions">
                              ...
                              {!readOnly && <button className="doc-action-btn delete" onClick={() => handleDelete(key as 'cdl' | 'medicalCard')}>Delete</button>}
                            </div>
                          </div>
                        ) : (
                          <div className="driver-doc-empty">
                            {readOnly
                              ? <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Auto-filled when applicant submits the form</p>
              ...
              <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {canHire && (
                  <button onClick={handleHire} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg, #48bb78, #38a169)', color: 'white', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>
                    <Emoji symbol="✅" size={16} /> Hired — Move to Drivers
                  </button>
                )}
                {!canHire && (
                  <p style={{ fontSize: '13px', color: 'var(--text-secondary)', textAlign: 'center' }}>
                    {!canHireDocs(modalDocs) ? '⚠️ Upload CDL and Medical Card to enable hiring'
                      : selectedDriver.equipment === 'Unsigned' ? '⚠️ Assign equipment before hiring'
                      : '⚠️ Status must be "Applied" to hire'}
                  </p>
                )}
                <button onClick={handleDeleteDriver} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #e53e3e', background: 'transparent', color: '#e53e3e', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>
